...
### DIALOG BOX ROUTINE ###
exp_info = {'participant_nr': '', 'age': '','number of trials':[10,50,100,150]}   #'': EMPTY BOX; [...]: DROP DOWN
...
"""
instruct=TextStim(win,instruct_txt,pos=(0,0.3))
instruct.draw()
win.flip()
...
def get_name():
    p_name=''
    kb = Keyboard()
    while True:           #Changed the loop to avoid adding 'return' to 'p_name'
                          #and now participants can use 'backspace' to correct typos
        keys = kb.getKeys()
        for key in keys:
            if key.name == 'return':
                return p_name # break out of the loop!
            elif key.name == 'backspace':
                p_name = p_name[:-1]
                display=TextStim(win,p_name,pos=(0,-0.2),color=(1, 0, -1))
                display.draw()
                instruct.draw()
                win.flip()
            else:
...
N_TRIALS = trialn
N_DOTS = 100
DOT_SIZE = 8  # in pixels
DOT_SPEED = 0.04  # degrees/frame
...
for coherence in COHERENCE_LEVELS:
    for direction in DIRECTIONS:
        ...
        for rep in range(0,int(trialn/10)):   #10:有十种condition；rep: reputation?
            trial_list.append({
                'coherence': coherence,
                'direction': direction,
...
end_text = TextStim(win, "Task complete! Thank you.", color=(-1,0,-1))
end_text.draw()
win.flip()
wait(5)         #core.wait()= wait()
